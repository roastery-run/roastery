...
import { receiveShipment, refreshContractPosition } from "../../lib/domain/contracts";
...
registerRpc(
  ...
  async (input, ctx) => {
    const id = await ctx.db.transaction(async (tx) => {
      ...
      await tx.emit({
        type: "sourcing.contract.created",
...
registerRpc(
  ...
  async (input, ctx) =>
    ctx.db.transaction(async (tx) => {
      const result = await receiveShipment(tx, input);
      await tx.emit({
        type: "sourcing.contract.shipment_received",
        resourceType: "shipment",
        resourceId: input.shipmentId,
        payload: {
          shipmentId: input.shipmentId,
          // The lots the shipment became. This is the join a receiving system
          // needs and cannot reconstruct from the shipment id alone.
          greenLotIds: result.createdLots.map((l) => l.id),
        },
      });
      return result;
    }),
