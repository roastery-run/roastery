...
import { parseArtisan } from "../../lib/domain/artisan";
import { applyInventoryTransaction, kg, recordTransformation } from "../../lib/domain/inventory";
import { buildPreview, deriveMetrics, flushRoast } from "../../lib/domain/roast-flush";
import { applyRoastedTransaction } from "../../lib/domain/roasted";
...
registerRpc(
  ...
  async (input, ctx) => {
    ...
    await ctx.db.emit({
      type: "production.roast_batch.started",
      resourceType: "roast_batch",
      resourceId: batch.id,
      payload: {
        id: batch.id,
...
registerRpc(
  ...
  async (input, ctx) => {
    ...
    await ctx.db.emit({
      type: "production.roast_batch.completed",
      resourceType: "roast_batch",
      resourceId: updated.id,
      payload: {
        id: updated.id,
        batchNumber: updated.batchNumber,
        chargeWeightKg: updated.chargeWeightKg,
        dropWeightKg: updated.dropWeightKg,
        weightLossPct: updated.weightLossPct,
        developmentTimeS: updated.developmentTimeS,
        dtrPct: updated.dtrPct,
      },
    });
