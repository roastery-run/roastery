...
function InputGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    // A <fieldset> would be the semantic element, but it cannot be styled as a
    // flex row without resetting a pile of user-agent styles, and this groups
    // one control with its adornments rather than a set of related fields.
    // biome-ignore lint/a11y/useSemanticElements: fieldset is not stylable here.
    <div
...
function InputGroupAddon({
  ...
  return (
    // Presentational: an icon or a unit label sitting beside the control. The
    // click handler is a convenience that widens the input's hit area, so it
    // needs no keyboard equivalent — Tab already reaches the input itself, and
    // adding a key handler here would put a second stop in the tab order for
    // something that is not focusable.
    // biome-ignore lint/a11y/noStaticElementInteractions: hit-area convenience on aria-hidden decoration.
    // biome-ignore lint/a11y/useKeyWithClickEvents: the input it focuses is already in the tab order.
    <div
